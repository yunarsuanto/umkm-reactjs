...
const DeleteModalLessonItem = ({ open }: DeleteModalLessonItemProps) => {
  ...
  const onSubmit = () => {
    mutate({id: selectedLessonItem.id}, {
      onSuccess: () => {
        dispatch(closeDeleteModal());
      },
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        {/* Header */}
        <h2 className="text-xl font-bold text-gray-900 mb-4">Hapus Lesson Item</h2>
        ...
        <p className="text-gray-700 mb-6">
          Apakah kamu yakin ingin menghapus <strong>{selectedLessonItem.content}</strong>?
        </p>
