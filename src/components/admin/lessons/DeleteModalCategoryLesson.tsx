...
const DeleteModalCategoryLesson = () => {
  const dispatch = useAppDispatch();
  const { selectedCategoryLesson, openDelete } = useAppSelector((state) => state.categoryLesson);
  ...
  const onSubmit = () => {
    mutate({ id: selectedCategoryLesson.id }, {
      onSuccess: () => {
        dispatch(closeDeleteModal());
      },
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        <h2 className="text-lg font-bold mb-4">Hapus Category</h2>
