...
const AdminLessonItemCreatePage = () => {
  ...
  const { image, thumbnail } = useAppSelector((state) => state.lessonItem)
  ...
  const handleSubmit2 = async (data: AddLessonItemSchema) => {
    if (!data.media){
      dispatch(setErrorFileNull('File Tidak Boleh Kosong'))
      return
    }else{
  ...
  return (
    <AdminLayout>
      <div className="p-8">
        {/* Header */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900">Add Lesson Item</h2>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 max-w-4xl">
          ...
          <form onSubmit={handleSubmit(handleSubmit2)} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              >
                {isPending ? 'Adding...' : 'Add Lesson Item'}
              </button>
            </div>
          </form>
        </div>
