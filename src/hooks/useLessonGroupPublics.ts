...
import { getLessonGroupPublic } from '@/api/lesson_items.api';
import { GetLessonGroupRequest, GetLessonGroupResponse } from '@/types/admin/lesson_item/GetLessonGroupTypes';

export const useLessonGroupPublics = (pagination: Pagination, req: GetLessonGroupRequest, options: {enabled: boolean}) => {
  return useQuery<GetLessonGroupResponse, ApiErrorType>({
    ...
    queryFn: () => getLessonGroupPublic(pagination, req),
    staleTime: 5 * 60 * 1000,
    gcTime: 1000 * 60 * 10,
